...
option_screen:
	...
	mov cx, 10100000b		; Colour: white text on light blue
	...
	os_init_msg		db 'QroOS ', QROOS_VER, ' ', QROOS_CODENAME, 0
	os_version_msg db '[Menu] ', 0 ; allocate space for the time string


mov bx, os_version_msg + 7 ; set BX to point to the start of the time string buffer
call os_get_time_string    ; call the function to fill in the time string

mov ecx, os_version_msg + 7 ; set ECX to point to the location after the '[Menu] ' prefix
movzx ebx, bx              ; zero-extend the 16-bit BX register to a 32-bit EBX register
cld                         ; clear the direction flag for string operations
rep movsb                   ; copy the time string to the message buffer
...
not_bas_extension:
	...
	kerndlg_string_1	db 'Cannot load and execute QroOS kernel!', 0
	kerndlg_string_2	db 'KERNEL.BIN is the core of QroOS, and', 0
